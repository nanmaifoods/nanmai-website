...
import {
  ...
  Settings, BarChart3, Globe, Bell, LogOut, Leaf, ChevronRight, Image
} from 'lucide-react';
...
const NAV = [
  ...
  { section: 'Content',
    items: [
      ...
      { href: '/admin/cms',       icon: Globe,           label: 'Site Content' },
    ...
    <aside className={clsx(
      ...
      <div className="flex items-center gap-3 px-4 py-5 border-b border-white/10">
        ...
        {!collapsed && (
          <div className="overflow-hidden">
            <div className="font-display font-bold text-base text-brand-pink leading-tight">NANMAI</div>
            <div className="text-[10px] text-brand-lime tracking-widest">ADMIN PANEL</div>
          </div>
        )}
        <button
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-2 space-y-6">
        {NAV.map(({ section, items }) => (
          <div key={section}>
            {!collapsed && (
              <p className="text-[10px] text-white/40 uppercase tracking-widest px-3 mb-2 font-semibold">{section}</p>
            )}
